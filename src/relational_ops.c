 ...
 * Version: relational_ops.c,v 1.3.2.1 2004/02/09 20:05:20 rleyton Exp
 *
 ...
 */
	...
	if ( (return_rel==NULL) || (rel==NULL) ) {
		...
		while (attrib!=NULL) {
			...
			if (database_datadictionary(db)>=NO_DD_RELATIONS) {
				if ( (status_tempdb==TRUE) && (configuration!=TRUE) && (strcmp(database_name(db),TEMPDB_NAME)!=0) && (return_rel->name[0]=='z') && (return_rel->name[1]=='z')) {
					ddmaintenance(tempdb,"add (%s) (%s,%s,STRING,%d)",LEAP_DD_ATTRIBUTES,relation_name(return_rel),
														   attribute_name(attrib), 
															attribute_size(attrib));
				} else {
					ddmaintenance(db,"add (%s) (%s,%s,STRING,%d)",LEAP_DD_ATTRIBUTES,relation_name(return_rel),
														   attribute_name(attrib), 
															attribute_size(attrib));
				}
			}
		...
		fclose(nf);
		nf=fopen(path,"r+b");
		relation_update_header(rname, noattribs, return_rel->temporary, return_rel->system, &nf);
		fclose(nf);
		nf=fopen(path,"r+b"); 
		res=fseek(nf,0,SEEK_END);  
			do_debug(DEBUG_ALL,"  fseek returned [%d]\n",res);
 ...
 */
	...
	if (rtrel==NULL) {
		...
		fclose(nf);
		nf=fopen(path,"r+b"); 
		relation_update_header(dest_name, count, rtrel->temporary,rtrel->system, &nf);
		fseek(nf,0,SEEK_END);
		fflush(nf);
 ...
 */
	relation rtrel;
	char *rname,rel_name[RELATION_NAME_SIZE+1]="",path[FILE_PATH_SIZE]="";
	attribute attr;
	unsigned int count;
	tuple ctuple;
	word anum;
	...
	if ( (rtrel==NULL) || (rel==NULL) ) {
		...
		fclose(nf);
		nf=fopen(path,"r+b"); 
		relation_update_header(rname, count, rtrel->temporary,rtrel->system, &nf);
		fseek(nf,0,SEEK_END);
		fflush(nf);
		relation_create_write_eoh_marker( count, &nf );
		...
		*attribmkr='\0';		
		...
		if (att!=NULL) {
			...
			while ( (ta!=NULL) && (attrib!=NULL) ) {
			 	...
			 	* for...
			 	*/
				do_debug(DEBUG_INFO,"No attribs: [%d]\n",rel->noattributes);
	...
	if ( (rel1!=NULL) && (rel2!=NULL) ) {
		...
		if (rtrel!=NULL) {
			...
			if (exceeded_limit==FALSE) {
				...
				fclose(nf);
				nf=fopen(path,"r+b");
				relation_update_header(rname,counter,rtrel->temporary,rtrel->system,&nf);
				fclose(nf);
				nf=fopen(path,"r+b");
				fseek(nf,0,SEEK_END);
				fflush(nf);
						...
						if (evaluate( condition, ct, NULL )==TRUE) {
							...
							while (strlen(string)>0) {
								...
								*assign='\0';
								...
								cut_token(set,',',string);
								do_debug(DEBUG_MODERATE,"slen %d\n",strlen(string)); 
							}
 ...
 */
	...
	tuple nt;
	char all[MAXIMUM_ALL_DATA],*citem;
