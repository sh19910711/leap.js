 ...
 * Version: attribs.c,v 1.204.2.1 2004/02/09 20:05:20 rleyton Exp
 *
...
int attr_fmt_size(attribute att) {
	int tabs=0,tmp,tmp2;
	char *spacing_var;
	...
	if ( (att->attrib_size>0) && (tabs==0) )
		if ( (attribute_type(att)==DT_BOOLEAN) ) {
			tmp=strlen(attribute_name(att));
			if (tmp<5) tmp=5;
			return(tmp);
		} else {
			tmp=att->attrib_size;
			tmp2=strlen(attribute_name(att));
			if (tmp2>tmp) {
				return(tmp2);
			} else {
				return(tmp);
...
char *fmt_build(char *fmt,attribute att,int *attrsize) {
/* fmt_build
 ...
 */
	...
	if ( (att->attrib_size>0) && (tabs==0) )
		if ( (attribute_type(att)==DT_BOOLEAN) ) {
			tmp=strlen(attribute_name(att));
			if (tmp<5) tmp=5;
			sprintf(fmt,"%%-%d.%ds ",tmp,tmp);
			*attrsize=tmp+1;
		} else {
			tmp=strlen(attribute_name(att));
			tmp2=att->attrib_size;
			*attrsize=att->attrib_size+1;
 ...
 */
	...
	do_debug(DEBUG_ENTER,"Start: attribute_display\n");
	...
	do_debug(DEBUG_ENTER,"End (Ok): attribute_display\n");
}
	
void attributes_printfromrel(relation rel) {
/* attribute_print
 * Print out all of the attributes associated with
 ...
 */
	attribute att;
	char fmt[FORMAT_SIZE];
	int counter,sizeofattrs=0,attrsize;
	tuple ctuple;
	word count;

	do_debug(DEBUG_ENTER,"Start: attributes_printFROMREL\n");
	...
	while (att!=NULL) {
		fmt_build(fmt,att,&attrsize);
/*		leap_printf("(%d)",attrsize);*/
		sizeofattrs+=attrsize;
	...
	for (counter=0;counter<sizeofattrs;counter++) leap_printf("-");

	leap_printf("\n");

	do_debug(DEBUG_ENTER,"End (Ok): attributes_printFROMREL\n");
}

void attributes_printtuple(tuple ctuple) {
/* attribute_print
 * Print out all of the attributes associated with
 * a relation - For rl_display mainly.
 */
	...
	do_debug(DEBUG_ENTER,"Start: attributes_printTUPLE\n");

	att=relation_attribute_getfirst(ctuple,&count);
...
/*		leap_printf("(%d)",attrsize);*/
		...
		/* Locate the next attribute */
		att=relation_attribute_getnext(ctuple,att,&count);
	}
...
/*		leap_printf("[%d]",attr_fmt_size(att));*/
		for(counter=0; counter<attr_fmt_size(att);counter++) leap_printf("-");
...
/*	for (counter=0;counter<sizeofattrs;counter++) leap_printf("-");
	...
	do_debug(DEBUG_ENTER,"End (Ok): attributes_printTUPLE\n");
}
 ...
 */
	...
	do_debug(DEBUG_ENTER,"Start: attribute_find\n");
	do_debug(DEBUG_INFO,"Relation: [%s] Attribute: [%s]\n",relation_name(rel),name);
	...
	if ( (catt!=NULL) && (name!=NULL) && (strcmp(attribute_name(catt),name)==0) ) {
		...
		do_debug(DEBUG_MODERATE,"Found attribute #%d [%s]\n",attribute_no(catt),attribute_name(catt));
		do_debug(DEBUG_ENTER,"End (Ok): attribute_find\n");
		...
		do_debug(DEBUG_ENTER,"End (Err): attribute_find\n");
		/* Return nothing */	
		return(NULL);
	}
}
