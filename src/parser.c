 ...
 * Version: parser.c,v 1.206.2.1 2004/02/09 20:05:20 rleyton Exp
 *
	...
	while ( (result_tree==NULL) && (abort==FALSE) )	 {
		...
		if (result_tree==NULL) {
			...
			switch (ptree->operation) {
				...
				case C_DISPLAY_REL:
					ptree->result=vprocess_query(db,"display(project(%s)(%s,%s,%s))",LEAP_DD_RELATIONS,LEAP_DDA_RELATIONS_NAME,LEAP_DDA_RELATIONS_TEMP,LEAP_DDA_RELATIONS_SYSTEM);
					break;
			  ...
			  case C_RECORD:
				...
				case C_ADD:
					ptree->result=insert(db,ptree->lresult,ptree->second);
					break;
				...
				case C_DESCRIBE:
					...
					if ((status_tempdb==TRUE) && (configuration!=TRUE) && (strcmp(database_name(db),TEMPDB_NAME)!=0) && (ptree->lresult->name[0]=='z') && (ptree->lresult->name[1]=='z')) {
						ptree->result=vprocess_query(tempdb,"display(project(select (%s) (%s=\"%s\")) (%s,%s,%s))",
						...
						ptree->result=vprocess_query(db,"display(project(select (%s) (%s=\"%s\")) (%s,%s,%s))",
						LEAP_DD_ATTRIBUTES,LEAP_DDA_ATTRIBUTES_RELATION,relation_name(ptree->lresult),
					    LEAP_DDA_ATTRIBUTES_ATTRIBUTE,LEAP_DDA_ATTRIBUTES_TYPE,LEAP_DDA_ATTRIBUTES_SIZE);
					}
					break;
...
/*					if (relation_system(ptree->lresult)!=TRUE) { */
						rl_compact(db,&ptree->lresult);
						ptree->result=ptree->lresult;
/*					} else {
						raise_error(ERROR_TAMPERSYSTEM,NONFATAL,relation_name(ptree->lresult));	
					}*/
					break;
				case C_DELREL:
					if (relation_system(ptree->lresult)!=TRUE) {
				...
				case C_CREATE:
					...
					/* Create database */
					tdb=LEAPAPI_db_init( tdir, ptree->first, TRUE );
					
					/* Close of the database to ensure hash tables etc. are flushed out
					 * and data dictionary is maintained
					 */
					relations_dispose_all(tdb);
					LEAPAPI_db_destroy(&tdb);
 ...
 */
	...
	while (ptree!=NULL) {
		...
		strip_trailing_spaces(ptree->expression);
 ...
 */ 
	...
	do_debug(DEBUG_ENTER,"OPERATOR Start: process_query [%s]\n",query);
	...
	if (!checkcomment(query))  {
		...
		do_debug(DEBUG_ENTER,"OPERATOR End (Ok): process_query [%s]\n",query);

		/* return relation */
		return(rtrel);
	} else {
		do_debug(DEBUG_ENTER,"OPERATOR End (Error): process_query [%s]\n",query);

		return(NULL);
	}
}
	
relation vprocess_query( database db,
						char *fmt, ... ) {
/* vprocess_query
 ...
 */
	...
	return( process_query(db,expression) );
